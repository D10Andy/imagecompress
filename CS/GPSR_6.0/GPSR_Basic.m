...
%                       当两个连续估计值之差的绝对值除以其中一个的绝对值低于tolA时停止
...
%               main phase of the algorithm.最大迭代次数
%               Default = 10000
%
%  'MiniterA' = minimum number of iterations performed in the
%               main phase of the algorithm.最小迭代次数
%               Default = 5     [5,10000]
%
...
%               Default = 5
%               debiasing 是去偏差的意思，在去偏差阶段，迭代的范围是[5,200]
%
...
% nargin是用来判断输入变量的个数
% ~= 是不等于
% varargin=可变长度输入参数列表，允许调用者调用该函数时根据需要来改变输入参数的个数
...
% 终止条件的形式，上面赋值是3,是使用LCP方法判断
if (sum(stopCriterion == [0 1 2 3 4])==0)
   error(['Unknown stopping criterion']);
end

% 函数句柄，将函数引用传递给另一个函数；减少定义您的功能的文件数量；提高重复操作的性能；在评估功能时确保可靠性
% 判断obj是否是class_name类型
